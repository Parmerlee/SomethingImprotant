 ...
 *
 */
public class DimensionTimeActivity extends BaseActivity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		graph_title_name.setText(acTitle.trim());
	...
	private ACLineGraphView initGraphView() {
		...
		if (data.size() != 0) {
			for (int i = 0, n = Math.min(graph_title_descId.length,
					colKeyList.length - 1); i < n; i++) {
				...
				handler.post(new Runnable() {
					...
					public void run() {
						...
						if (!mLastDate.equals(date)) {
							...
							mLastDate = date;
							for (int i = 0, n = Math.min(
									graph_title_descId.length,
									colKeyList.length - 1); i < n; i++) {
								TextView tv = (TextView) findViewById(graph_title_valueId[i]);
								TextView tvU = (TextView) findViewById(graph_title_unitId[i]);
	...
	private class DataTask extends AsyncTask<Void, Void, Object> {
		...
		protected void onPreExecute() {
			super.onPreExecute();
			showMessageDailog(DimensionTimeActivity.this);
		}
		...
		protected void onPostExecute(Object result) {
			...
			if (data != null && data.size() != 0) {
				...
				leftAdapter = new KpiPeriodLeftAdapter(
						DimensionTimeActivity.this, data, colKeyList);
				rightAdapter = new KpiPeriodRightAdapter(
						DimensionTimeActivity.this, data, kpiInfo, colKeyList,
						colInfoMap);
				...
				toast(DimensionTimeActivity.this, getString(R.string.no_data));
			}
			pDialog.dismiss();
		}
	}
	...
	private void changeTitle(String[] namesRight) {
		...
		KpiHasChartRightView ret = new KpiHasChartRightView(
				DimensionTimeActivity.this, R.layout.title_right_item, str, 1,
	...
	public void onViewItemClick(int position) {
		...
		if ("1".equals(showTop5Flag) && !"1".equals(areaLevel)) {
			Intent intent = new Intent(DimensionTimeActivity.this,
