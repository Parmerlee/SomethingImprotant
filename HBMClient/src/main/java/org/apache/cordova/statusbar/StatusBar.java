	...
	public void initialize(final CordovaInterface cordova,
		...
		this.cordova.getActivity().runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				// #000000.
				setStatusBarBackgroundColor(preferences.getString(
	...
	public boolean execute(final String action, final CordovaArgs args,
		...
		if ("_ready".equals(action)) {
			...
			callbackContext.sendPluginResult(new PluginResult(
					PluginResult.Status.OK, statusBarVisible));
			return true;
		}
		...
		if ("show".equals(action)) {
			this.cordova.getActivity().runOnUiThread(new Runnable() {
				@Override
				public void run() {
					// SYSTEM_UI_FLAG_FULLSCREEN is available since JellyBean,
					// but we
					// use KitKat here to be aligned with "Fullscreen"
					// preference
					if (Build.VERSION.SDK_INT >= 19) {
						int uiOptions = window.getDecorView()
			...
			this.cordova.getActivity().runOnUiThread(new Runnable() {
				...
				public void run() {
					try {
						...
						LOG.e(TAG,
								"Invalid hexString argument, use f.i. '#777777'");
					}
				}
			});
			return true;
		...
		if (Build.VERSION.SDK_INT >= 21) {
			if (colorPref != null && !colorPref.isEmpty()) {
				...
				try {
					...
					LOG.w(TAG,
							"Method window.setStatusBarColor not found for SDK level "
