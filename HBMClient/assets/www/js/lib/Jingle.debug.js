...
/*zl add*/
    var _showPopup = function( hash ){
       // J.hasPopupOpen?J.closePopup();J.popup(hash);

        console.log( hash );
        var ztime = J.Cache.get('stime').data;
        var zarea = J.Cache.get('sarea').data;
        var zareaCode = zarea.AREA_CODE;
        //console.log( ztime.data );
        ...
        console.log( timestr );
         ...
         if( timestr.length >=8 ){
            year = Number(timestr.substring(0,4) );
            month =  Number( timestr.substring(4,6) );
            if( month >0 ){ month = month -1;}
            day = Number( timestr.substring(6,8) );
         }else if(timestr.length >0 ){
            year = Number(timestr.substring(0,4) );
            month =  Number( timestr.substring(4,6) );
         }

         var param = J.Cache.get('timeparam').data; 
         console.log( param );
         var timefunc;
         if( param == '#jiakuan_article'){
            timefunc = jiakuan.getdatabytime;
         }else if( param == '#home_network_article'){
            timefunc = homenetwork.getdatabytime;
         }

         param = J.Cache.get('areaparam').data; 
         var areafunc;
         if( param == '#jiakuan_article'){
            //areafunc = jiakuan.getdatabyarea;
         }else if( param == '#home_network_article'){
            //areafunc = homenetwork.getdatabyarea;
         }
        ...
        switch( hash ){
            ...
            J.popup({
                ...
                onShow : function(){
                    new J.Calendar('#popup_calendar',{
                        ...
                        //$('#jiakuan_article footer a:first').text(date);
                        console.log('时间选择' + date );
                        $('#stime span').text(date);
                        //jiakuan.getdatabytime( date );
            ...
            break;
            case '#area' :
                jiakuan.getareadata( zareaCode, { onSelect: function( param ){
                        //console.log( param );
                    J.popup({
                    ...
                    onShow: function(){
                        new J.Area('#popup_area',{
                            ...
                            onSelect:function( areaCode ,areaName ){
                                ...
                                $('#sarea span').text(areaName);

                                var areadat = { AREA_CODE:areaCode,  AREA_NAME:areaName };
                                J.Cache.save('sarea', areadat);
                                areafunc(areaCode);
                                J.closePopup();
                        }
                    })
...
J.Toast = (function($){
    var toast_type = 'toast',_toast,timer,
        ...
        TEMPLATE = {
            ...
            loading : '<a href="#" style="border-radius:8px;background-color:#3498DB"><i class="icon spinner left"></i>{value}</a>'
        }
...
J.Popup = (function($){
    ...
    var loading = function(text){
        var markup = TEMPLATE.loading.replace('{title}',text||'数据加载中...');
        show({
            html : markup,
            pos : 'loading',
            opacity :.1,
...
;(function($){
    ...
    var calendar = function(selector,options){
        ...
        var _init = function(){
            ...
            console.log( 'init' );
            console.log( _this.settings.date );
            console.log( _this.settings.date.getDate() );
        ...
        /*获取下月份总天数*/
        var _daysInNextMonth = function(date){
            return ( new Date(date.getFullYear(),date.getMonth()+1,1) ).getDate();
        }

        /*获取上月份总天数*/
        var _daysInPreMonth = function(date){
            return ( new Date(date.getFullYear(),date.getMonth()-1,1) ).getDate();
        }
        ...
        var _subscribeEvents = function(){
            ...
            $el.on('tap',function(e){
                ...
                if($target.is('[data-year].next')){
                    ...
                    var monthd = currentDate.getDate();
                    var nextmonthd = _daysInNextMonth(currentDate);

                    // console.log( currentDate );
                    // console.log('当月天数' + monthd );
                    // console.log('后一月天数' + nextmonthd );

                    if( monthd > nextmonthd ){
                        console.log('后一月天数' + nextmonthd );
                        currentDate = new Date(currentDate.getFullYear(), currentDate.getMonth()+1,nextmonthd );
                    }else{
                    ...
                    //后一月
                    //currentDate.setMonth(currentDate.getMonth()+1);
                    //console.log('后一月' + currentDate );
                    _this.refresh(currentDate);
                }else if($target.is('[data-month].previous')){

                    var monthd = currentDate.getDate();
                    var nextmonthd = _daysInPreMonth(currentDate);
...
;(function($){
     ...
     * @param options 配置参数
     */
    var calendarMonth = function(selector,options){
        var defaults = {
                months : ["1月", "2月", "3月", "4月", "5月", "6月",
                    "7月", "8月", "9月", "10月", "11月", "12月"],
                curYearMonth : new Date(),//日历当前月份
                onRenderDay : undefined,//渲染单元格时的事件
                onSelect : undefined //选中日期时的事件
            },
            _this = this,
        ...
        var _render = function(){
            var html = '';
            html += '<div class="jingle-calendar">';
            html += _renderNav(currentYear);
            html += '<div class="jingle-calendar-body">';
            html += _renderBody(currentYear,currentMonth);
            html += '</div></div>'
            $el.html(html);
        ...
        var _renderNav = function(year){
            var html = '<div class="jingle-calendar-nav">';
            html += '<div> <i class="icon previous" data-year='+year+'></i><span>'+year+'</span> 年<i class="icon next" data-year='+year+'></i></div>';
            html += '</div>';

            return html;
        }

        var _renderBody = function(year, month ){
            ...
            for( var i = 0; i < 12;i=i+4 ){
                ...
                html += '<tr>';
                    ...
                    monthstr = _this.settings.months[i];
                    month = i+1;
                    yearmonth = String(year);
                    if(month < 10 ){
                        yearmonth += '0'+ String(month);
                    }else{
                        yearmonth += String(month);
                    }
                    yearmonth = _this.format(yearmonth);   
                    html += '<td data-selected="selected" class="'+classname+ '" data-date= '+ yearmonth +'>'+monthstr+'</td>';
                    ...
                    monthstr = _this.settings.months[i+1];
                    month = i+2;
                    yearmonth = String(year);
                    if(month < 10 ){
                        yearmonth += '0'+ String(month);
                    ...
                    monthstr = _this.settings.months[i+2];
                    month = i+3;
                    yearmonth = String(year);
                    if(month < 10 ){
                        yearmonth += '0'+ String(month);
                    ...
                    monthstr = _this.settings.months[i+3];
                    month = i+4;
                    yearmonth = String(year);
                    if(month < 10 ){
                        yearmonth += '0'+ String(month);
        ...
        this.refresh = function(date){
            //console.log(date);
            $yearText.text(date.getFullYear());
            var newbody = _renderBody(date.getFullYear(),date.getMonth());
            $calendarBody.html(newbody);
        }
        _init();
    }
     ...
     * @return {*}
     */
    calendarMonth.prototype.parse = function(date){
        var dateRE = /^(\d{4})(?:\-|\/)(\d{1,2})(?:\-|\/)(\d{1,2})$/;
        return dateRE.test(date) ? new Date(parseInt(RegExp.$1, 10), parseInt(RegExp.$2, 10) - 1, parseInt(RegExp.$3, 10)) : null;
    }
    /**
     ...
     * @return {String}
     */
    calendarMonth.prototype.format = function( yearMonth ){

        var timestr;
        if( yearMonth.length >=6 ){
           timestr = yearMonth.substring(0,4) + '-' +  yearMonth.substring(4,6);
         }
        return timestr;
    }

    J.CalendarMonth = calendarMonth;
...
;(function($){
        ...
        var area = function(selector,options){
        var defaults = {
                onSelect : undefined //选中日期时的事件
            },
            _this = this,
        ...
        var _init = function(){
            console.log('_init');
            _this.settings = $.extend({},defaults,options);
            jsondata = _this.settings.areadata;
            _render();
            _subscribeEvents();
        }
         ...
         var _render = function(){
            ...
            html += '<div class="jingle-calendar" >';
            html += _renderHead();
            html += '<div class="active" data-scroll="true" style="height: 300px;text-align: center;font-size: 20px;\
                            font-weight: 600;margin-top: 10px;color:#E74C3C "><ul class="list">';
            html += _renderBody();
            html += '</ul></div></div>'
            $el.html(html);
        ...
        var _renderHead = function(){
            var html = '<div><h3 style="text-align:center;font-size:1.0em">上下滑动选择地域</h3></div>';
            return html;
        }
        ...
        var _renderBody = function(){
            ...
            for(var i=0; i < jsondata.length; ++i){
                      var area_name = jsondata[i].AREA_NAME;//data-areaName='+area_name+'
                      var area_code = jsondata[i].AREA_CODE;
                      html +='<li data-selected="selected" data-area='+area_code+' data-code='+area_name+'>'+area_name+'</li>';
            }
            return html;
        }
        ...
        var _subscribeEvents = function(){
            ...
            $el.on('tap',function(e){
                ...
                if($target.is('li')){
                    ...
                    if(areaStr && _this.settings.onSelect){
                        _this.settings.onSelect.call(_this,areaStr,areaName);
                    }
                }
            });
        }
