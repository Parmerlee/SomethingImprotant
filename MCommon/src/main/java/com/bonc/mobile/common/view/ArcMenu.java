...
public class ArcMenu extends ViewGroup implements OnClickListener{
	...
	private static final float FLOAT_ALPHA = 0.3f;
	...
	public void setDates(int tag, List<Integer> list) {
		int count = getChildCount();
		int length = Math.min(list.size(), count - 1);
		for (int i = 0; i < length; i++) {
			View child = getChildAt(i + 1);
			child.setTag(tag, list.get(i).toString());
			child.setBackgroundResource(list.get(i));
		}
		for (int i = 0; i < Math.abs(list.size() - count - 1); i++) {
			removeView(getChildAt(length + 1));
		}
		invalidate();
	}
	...
	public void toggleMenu(int duration) {
		...
		for (int i = 0; i < count - 1; i++) {
			...
			AlphaAnimation alphaAnima = new AlphaAnimation(alpha_start,
					alpha_end);
			alphaAnima.setDuration(duration);
			alphaAnima.setFillAfter(false);

			animset.addAnimation(rotateAnim);
			animset.addAnimation(tranAnim);
			animset.addAnimation(alphaAnima);
			...
			childView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					menuItemAnim(pos - 1);
					changeStatus();
					// ArcMenu.this.setAlpha(FLOAT_ALPHA);
